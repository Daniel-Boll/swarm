...
int main() {
  ...
  server.registerPacketHandler<network::PlayerPositionPacket>(
      [&server](const network::PlayerPositionPacket& packet, ENetPeer* sender) {
        server.broadcastToOthers<network::PlayerPositionPacket>({packet, sender->connectID},
                                                                sender);
      });

  server.registerPacketHandler<network::SyncNewPlayerPacket>(
      [&server](const network::SyncNewPlayerPacket& packet, ENetPeer* sender) {
        auto relayed_packet = network::SyncNewPlayerPacket(packet, sender->connectID);
        auto recipient = server.getPeer(packet.to);

        server.sendPacket(relayed_packet, recipient);
      });
